...
def evaluate_trackingVisDrone(sequences, track_dir, gt_dir):
    all_info = []
    for seqname in sequences:
        track_res = os.path.join(track_dir, seqname + "-GT" +'.txt')
        gt_file = os.path.join(gt_dir, seqname + '.txt')
        ...
        metrics, extra_info = evaluate_sequence(trackDB, gtDB, distractor_ids=np.array([-1]))
...
def evaluate_trackingMOT(sequences, track_dir, gt_dir):
    all_info = []
    for seqname in sequences:
        # track_res = os.path.join(track_dir, seqname + '.txt')
        # gt_file = os.path.join(gt_dir, seqname + '/gt/gt.txt')
        track_res = os.path.join(track_dir, seqname + '-GT' + '.txt')
        gt_file = os.path.join(gt_dir, seqname + '-DPM'  + '/gt/gt.txt')
        assert os.path.exists(track_res) and os.path.exists(gt_file), \
            'Either tracking result {} or '\
            'groundtruth directory {} does not exist'.format(
                track_res, gt_file)
        ...
        metrics, extra_info = evaluate_sequence(trackDB, gtDB, distractor_ids=distractor_ids)
        print_metrics(seqname + ' Evaluation', metrics)
        all_info.append(extra_info)
    all_metrics = evaluate_bm(all_info)
    print_metrics('Summary Evaluation', all_metrics)
...
def parse_args():
    ...
    parser.add_argument(
        '--data_name', help='dataset name', type=str)
...
if __name__ == '__main__':
    ...
    if args.data_name == 'visdrone':
        evaluate_trackingVisDrone(sequences, args.track, args.gt)
    elif args.data_name == 'mot':
        evaluate_trackingMOT(sequences, args.track, args.gt)
